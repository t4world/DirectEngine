...
bool TransparentShaderClass::Render(ID3D11DeviceContext* deviceContext, int indexCount, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture,float alpha)
{
	bool result;
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture,alpha);
	if (!result)
	{
		return false;
	}
...
bool TransparentShaderClass::InitializeShader(ID3D11Device* device, HWND hwnd, WCHAR* vsFilename, WCHAR* psFilename)
{
	...
	D3D11_BUFFER_DESC transparentDesc;
	...
	transparentDesc.Usage = D3D11_USAGE_DYNAMIC;
	transparentDesc.ByteWidth = sizeof(TransparentBufferType);
	transparentDesc.BindFlags = D3D11_BIND_CONSTANT_BUFFER;
	transparentDesc.CPUAccessFlags = D3D11_CPU_ACCESS_WRITE;
	transparentDesc.MiscFlags = 0;
	transparentDesc.StructureByteStride = 0;

	result = device->CreateBuffer(&transparentDesc, NULL, &m_transparentBuffer);
	if (FAILED(result))
	{
		return false;
	}
...
bool TransparentShaderClass::SetShaderParameters(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView* texture,float alpha)
{
	HRESULT result;
