...
bool FontShaderClass::InitializeShader(ID3D11Device *device, HWND hwnd, WCHAR *vsShaderFile, WCHAR *psShaderFile)
{
	HRESULT result;
	...
	pixelShaderBuffer = 0;
	result = D3DX11CompileFromFile(vsShaderFile, NULL, NULL, "FontVertexShader", "vs_5_0", D3D10_SHADER_ENABLE_STRICTNESS, 0, NULL, &vertexShaderBuffer, &errorMessage, NULL);
	if (FAILED(result))
	{
		if (errorMessage)
		{
	...
	//This setup needs to match the VertexType,
	...
	//Create the pixel constant buffer pointer so we can access the pixel shader constant buffer from 
...
void FontShaderClass::OutputShaderErrorMessage(ID3D10Blob *errrorMessage, HWND hwnd, WCHAR *shaderFileName)
{
	char *compileError;
	unsigned long bufferSize;
	unsigned long i;
	ofstream fout;
	compileError = (char *)(errrorMessage->GetBufferPointer());
	//Get the length of the message.
	bufferSize = errrorMessage->GetBufferSize();
	...
	errrorMessage->Release();
	errrorMessage = 0;
	MessageBox(hwnd, L"Shader compiler error,please check shader-error.txt for detail message! ", shaderFileName, MB_OK);
}
...
bool FontShaderClass::SetShaderParameters(ID3D11DeviceContext *deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX viewMatrix, D3DXMATRIX projectionMatrix, ID3D11ShaderResourceView *texture, D3DXVECTOR4 pixelColor)
{
	...
	//Lock the pixel constant buffer so it can be writenn to
